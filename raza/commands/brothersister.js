...
module.exports.run = async ({ api, event }) => {
  ...
  try {
    ...
    const circleOne = await makeCircularImage(avatarOne, 115);
    const circleTwo = await makeCircularImage(avatarTwo, 115);

    const template = await Jimp.read(templatePath);

    template.composite(circleOne, 78, 70);
    template.composite(circleTwo, 310, 70);
